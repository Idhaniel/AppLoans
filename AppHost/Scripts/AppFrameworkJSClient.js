...
var formTypeName = "";
var formCaption = "";
var originalFormCaption = "";
var rowselectorwidth = "60px";
...
function connect() {
    ...
    __websocket = new WebSocket(websocketprotocol + "://" + location.hostname + ":" + port + "/");
    __websocket.onopen = function () {
        __websocket.send(JSON.stringify({ "token": antiforgerytoken }));
...
function deRegisterHook(hookName, callback) {
    if (hooks[hookName]) {
        let temphooks = hooks[hookName];
        let callbackindex = temphooks.indexOf(callback);
        if (callbackindex != -1) {
            temphooks.splice(callbackindex, 1);
        }
    }
}
...
function callHook(hookName) {
    if (hooks[hookName]) {
        for (let i = 0; i < hooks[hookName].length; i++) {
            let fn = hooks[hookName][i];
...
function deleteViewCookie(cname) {
...
function openView(html, viewid, dialog, containerstyle, caption, formtypename, done) {
    ...
    document.body.setAttribute('formtypename', formTypeName);
    prepareLookup(doc);
...
function infolog(message, type) {
    ...
    //if (infologmessagesdiv) 
    {
        ...
        let infologdivs = self.top.document.querySelectorAll('.infodiv');
        while (infologdivs.length > 1000) {
            let topinfologdiv = infologdivs[0];
            topinfologdiv.remove();
            infologdivs = self.top.document.querySelectorAll('.infodiv');
        }
...
function compare(iframe1, iframe2, desc) {
    let multiplier = 1;
    ...
    iframe1zindex *= multiplier;
    iframe2zindex *= multiplier;
...
function moveViewToBottom(iframeid) {
    var temp = self.top.document.querySelectorAll("div.iframecontainerdiv");
    var iframes = [];
    for (var i = 0; i < temp.length; i++) {
        iframes.push(temp[i]);
    }
    iframes = iframes.sort(function (iframe1, iframe2) {
        return compare(iframe1, iframe2, true);
    });

    var iframe = self.top.document.getElementById(iframeid);
    var indexofiframe = iframes.indexOf(iframe);

    var bottomiframe = iframes[iframes.length - 1];
    setViewZIndex(iframe, bottomiframe.style.zIndex);
    //debugger;
    for (var i = iframes.length - 1; i > indexofiframe; i--) {
        if (iframes[i].getAttribute('iframeid') != iframe.id) {
            var newZindex = iframes[i].style.zIndex ? parseInt(iframes[i].style.zIndex) + 1 : 1;
...
function prepareInfolog() {
    let infologdiv;
    let infologalreadyexists = false;
    if (infologdivid) {
        //debugger;
        infologdiv = document.getElementById(infologdivid);
        infologalreadyexists = true;
        if (!infologdiv) {
    ...
    var copyinfologbutton = document.createElement('button');
    copyinfologbutton.innerText = 'Copy Log';
    copyinfologbutton.setAttribute('class', 'copyinfologbutton');
    copyinfologbutton.setAttribute('id', 'copyinfologbutton');

    copyinfologbutton.addEventListener("click", function (event) {
        //innercontaineriframe.contentWindow.callFormMethod("clearInfolog");
...
function showLookup(entity, entityname, fieldgroup, loadableelementid, lookupcolumn, lookupelementid) {
    ...
    captionspan.innerText = 'Select ' + entityname;

    gotoa.setAttribute('entity', entity);
...
function prepareLookup(document) {
    ...
    let captionspan = document.createElement('div');
    captionspan.innerText = 'Select ';
    captionspan.setAttribute('id', '__lookup_captionspan');
    captionspan.setAttribute('class', 'lookupcaptionspan');

    let gotospan = document.createElement('div');
    gotospan.setAttribute('id', '__lookup_gotospan');

    let gotoa = document.createElement('a');
    gotoa.setAttribute('id', '__lookup_gotoa');
    gotoa.setAttribute('href', 'javascript:void(0)');
    gotoa.innerText = 'Go to main form';
    ...
    //__j(middiv).height()
    gotospan.appendChild(gotoa);

    captiondiv.appendChild(captionspan);
    captiondiv.appendChild(gotospan);
...
function sendNoOp() {
    if (!windowPaused()) {
        sendMessage(JSON.stringify({ messagetype: "noop", message: {} }));
    }
}
...
function forwardMessage(message) {
    ...
    for (var i = 0; i < iframes.length; i++) {
        ...
        let canforwardtoiframe = false;
        try {
            canforwardtoiframe = iframe && iframe.contentWindow && iframe.contentWindow.__forwardMessage != null;
        }
        catch (error) {
            console.log(error);
        }
        if (canforwardtoiframe) {
...
function onmessage(message) {
    ...
    try {
        ...
        if (data.messagetype != "eventreply" && data.messagetype != "methodcallreply" && data.containeriframeid && (self.frameElement == null || self.frameElement.id != data.containeriframeid)) {
            ...
            switch (data.messagetype) {
                ...
                case "loading":
                    ...
                    let loadingevent = document.createEvent("CustomEvent");
                    loadingevent.initCustomEvent("loading", false, false, null);
                ...
                case "close":
                    ...
                    }, 60000);//60 seconds should be more than enough time to finsh up what the iframe was doing e.g sending a reply or whatever
        ...
        console.log("Original Message: " + message.data);
...
function setCustomElementAttributes(element, fieldinfo) {
    ...
    if (fieldinfo.entityindex !== null && fieldinfo.entityindex !== undefined) {
        element.setAttribute("entityindex", fieldinfo.entityindex);
    }
    if (fieldinfo.fieldname) {
        element.setAttribute("fieldname", fieldinfo.fieldname);
    }

    if (fieldinfo.edt) {
        element.setAttribute("edt", fieldinfo.edt);
    }
...
class AppOperationsControl extends EntityElement {
    ...
    createOperations(operation, parentul, level) {
        ...
        if (operation.class) {
            __j(operationsli).addClass(operation.class);
        }
        ...
        if (operation.class) {
            __j(operationelement).addClass(operation.class);
        }
        ...
        if (operation.nopermission) {
            operationelement.setAttribute('nopermission', '');
            operationsli.setAttribute('nopermission', '');
        }
...
class AppFieldGroup extends EntityElement {
    ...
    createFields(fields) {
        ...
        for (let i = 0; i < fields.length; i++) {
            ...
            let fieldhostdiv = this.createField(i);
...
class AppGrid extends LoadableElement {
    ...
    connectedCallback() {
        ...
        if (!this.connected) {
            ...
            this.headandbodycontainer.addEventListener("scroll", function (event) {
                ...
                //let rowselectortds = __j(thiselement.shadowRoot.querySelectorAll(".app-grid-rowselector_td"));
                //let buttonstds = __j(thiselement.shadowRoot.querySelectorAll(".grid_edit_save_deleted_td"));

                //for (var i = 0; i < rowselectortds.length; i++) {
                //    __j(rowselectortds[i]).css("left", this.scrollLeft);
                //}

                //for (var i = 0; i < buttonstds.length; i++) {
                //    __j(buttonstds[i]).css("left", this.scrollLeft);
                //}
            });
            ...
            this.bodycontainer.setAttribute("class", "app-grid-table-body-container");
            this.bodycontainer.setAttribute('tabindex', '0');
            ...
            //debugger;
            let style = ".header_div { width: 100px; }";
            style += "table { table-layout: fixed;  border-spacing: 0;  border-collapse: collapse; margin-left: 0px; margin-right: 0px }";
            style += "th, td { padding-left: 0px; padding-right: 0px; }";
            ...
            this.bodycontainer.addEventListener('keydown', function (e) {
                if (e.key == 'ArrowUp' || e.key == 'ArrowDown') {
                    e.preventDefault();
                    //console.log(e);
                    if (e.key == 'ArrowDown') {
                        thiselement.moveNext();
                    }
                    else {
                        thiselement.movePrevious();
                    }
                }
    ...
    setRowSNos() {
        //this.id + "_sno_num_span_" + pageno + "_" + rowno
        let pagenos = Object.keys(this.tbodies);
        pagenos.sort();
        let sno = 0;
        for (let i = 0; i < pagenos.length; i++) {
            //iterate over each row of each page
            let pageno = pagenos[i];

            for (let rowno = 0; rowno < this.alldata[pageno].length; rowno++) {
                let snospan = this.shadowRoot.getElementById(this.id + "_sno_num_span_" + pageno + "_" + rowno);
                snospan.innerText = (++sno).toLocaleString();
            }
        }
    }

    setDataInfo(info) {
        let datainfodiv = this.shadowRoot.getElementById(this.id + "_data_info_div");
        if (datainfodiv) {
            datainfodiv.innerText = info;
        }
    }
    ...
    loadData(pageno, data) {
        ...
        for (let i = 0; i < data.length; i++) {
            //debugger;
            let tr = this.createRow(pageno, i, data[i]);
        ...
        let loadmoreinterval = setInterval(function () {
            if (!windowPaused()) {
                clearInterval(loadmoreinterval);
                //debugger;
                //if (thiselement.needsToLoadUp()) {
                //    thiselement.dispatchLoadEvent("up");
                //}
                if (thiselement.needsToLoadDown()) {
                    thiselement.dispatchLoadEvent("down");
                }
            }
        }, 100);
    }
    ...
    setColumnWidth(col) {
        ...
        let width = __j(header).outerWidth();
    ...
    //}
    dispatchDeSelectAllEvent() {
        let newevent = document.createEvent("CustomEvent");
        newevent.initCustomEvent("deselectall", false, false, null);
        this.dispatchEvent(newevent);
    }

    dispatchSelectAllEvent() {
        let newevent = document.createEvent("CustomEvent");
        newevent.initCustomEvent("selectall", false, false, null);
        this.dispatchEvent(newevent);
    }
    ...
    multiSelectRow(selected, pageno, rowno) {
    ...
    scrollToRowIfNeeded(pageno, rowno) {
        let radiobutton = this.shadowRoot.getElementById(this.id + "_" + pageno.toString() + "_" + rowno + "_" + "tr_selector_radiobutton");
        if (radiobutton) {
            ...
            let container = __j(this.headandbodycontainer);
            var contHeight = container.height();
            var contTop = container.scrollTop();
            var contBottom = contTop + contHeight;

            var elemTop = $(radiobutton).offset().top - container.offset().top;
            var elemBottom = elemTop + $(radiobutton).height();

            var isTotal = (elemTop >= 0 && elemBottom <= contHeight);

            if (!isTotal) {
                this.scrollToRow(pageno, rowno);
            }
        }
    }
    ...
    editMode(pageno, rowno, edit, editablefields) {
        //if (edit && this.ineditmode) {
        //    this.editMode(pageno, rowno, false, editablefields);
        //}
        //if (edit) {
        //debugger;
        let thiselement = this;
        let firsteditorelement;
        let rowdata = this.alldata[pageno][rowno];
            ...
            if (edit) {
                ...
                if (editablefields.indexOf(fieldinfo.field) >= 0) {
                    ...
                    if (editorelement.inputelement) {
                        editorelement.inputelement.addEventListener('focus', function (e) {
                            thiselement.lastfocusedcolumn = col;
                        });
                    ...
                    if (!firsteditorelement) {
                        firsteditorelement = editorelement;
                    }
                    ...
                    this.callHook("fieldEditMode", editorelement);

                    
                    //console.log(col + "," + this.lastfocusedcolumn);
                    if (col == this.lastfocusedcolumn && editorelement.inputelement) {
                        editorelement.inputelement.focus();
                    }

                    //grid_1_0_24_host_div app-grid-td-edit-div

                    //let hostdivid = this.id + "_" + this.selectedpageno + "_" + this.selectedrowno + "_" + col + "_host_div";

                }

                if (this.lastfocusedcolumn == -1 && firsteditorelement && firsteditorelement.inputelement) {
                    firsteditorelement.inputelement.focus();
                }
            }
            else {
                //first save the data
    ...
    removeRow(pageno, rowno) {
        ...
        tr.remove();
        let datarows = this.alldata[pageno];
        datarows.splice(rowno, 1);
        //debugger;
        if (this.alldata[pageno].length == 0) {
            delete this.alldata[pageno];
        }
    ...
    createRow(pageno, rowno, rowdata) {
        this.lastfocusedcolumn = -1;
        let trid = this.id + "_" + pageno.toString() + "_" + rowno + "_tr";
        let tr = this.shadowRoot.getElementById(trid);
        if (tr) {
        ...
        let snonumspan = document.createElement('span');
        snonumspan.setAttribute('class', 'app_grid_sno_span');
        snonumspan.setAttribute('id', this.id + "_sno_num_span_" + pageno + "_" + rowno);
        ...
        rowselectordiv.appendChild(snonumspan);
        ...
        //}
        //__j(editsavedeletediv).css('width', (!thiselement.noedit ? grideditsavedeletetdwidth : "0px"));
        ...
        //empty space is because the columns for the radio button and the buttons (which we want to remain fixed) will be set to position: absolute. this will cause the remaining columns to start from position 0
        //the empty space td is added to push the non fixed columns to the same offset where they would ordinarily be if we hadn't set position: absolute
        //let emptyspacetd = document.createElement('td');
        //emptyspacetd.setAttribute('class', 'app_grid_empty_space_td');
        //let actualgridsavedeletedwidth = !thiselement.noedit ? grideditsavedeletetdwidth : "0px";
        //__j(emptyspacetd).css("width", "calc(" + actualgridsavedeletedwidth + " + " + rowselectorwidth + ")");
        //__j(emptyspacetd).css("min-width", "calc(" + actualgridsavedeletedwidth + " + " + rowselectorwidth + ")");
        //tr.appendChild(emptyspacetd);
    ...
    getFirstFocusableChildElement(element) {
        let childNodes = element.shadowRoot != null ? element.shadowRoot.childNodes : element.childNodes;
        for (let i = 0; i < childNodes.length; i++) {
            let childNode = childNodes[i];
            if (childNode.tagName && (childNode.tagName.toLowerCase() === "input" || childNode.tagName.toLowerCase() === "textarea" || childNode.tagName.toLowerCase() === "select" || childNode.tagName.toLowerCase() === "textarea")) {//checkbox and date input still not focusing properly
                return childNode;
            }
            else {
                let ret = this.getFirstFocusableChildElement(childNode);
                if (ret) {
                    return ret;
                }
            }
    ...
    createFieldTd(tr, i, col, pageno, rowno, rowdata) {
        ...
        td.addEventListener('focus', function (e) {
            thiselement.lastfocusedcolumn = col;
            //debugger;
            if (thiselement.ineditmode) {
                let element = thiselement.getFirstFocusableChildElement(td);
                if (element) {
                    element.focus();
                }
            }
        ...
        if (displayelement.tagName.toLowerCase() == "app-date-display" || displayelement.tagName.toLowerCase() == "app-date-time-display") {
            __j(tdhostdiv).css("text-overflow", "unset");
        }
    ...
    createFields(fields) {
        ...
        for (let i = 0, col = 0; i < fields.length; i++ , col++) {
            let createdisplaycolumnsfunction = function () {
                ...
                while (this.displaycolumns.indexOf(col) >= 0) {
                    ...
                    thtemp.setAttribute('class', 'header');
                    ...
                    thtempdiv.setAttribute('title', thtempdiv.innerText);
            ...
            thdiv.setAttribute('title', thdiv.innerText);
        ...
        this.controlstd.setAttribute("colspan", this.fieldinfos.length + 1 + this.displaycolumns.length);
            ...
            if (!self.frameElement.opened) {
                ...
                if (loaded) {
                    for (let i = 0; i < thiselement.headerths.length; i++) {
                        ...
                        let width = __j(thdiv).outerWidth();
                        if (thiselement.columnsizes[i] !== null && thiselement.columnsizes[i] !== undefined) {
                            if (width != thiselement.columnsizes[i]) {
                                ...
                                let formname = document.body.hasAttribute('entitytypename') ? document.body.getAttribute('entitytypename') : document.body.getAttribute('formtypename');
                                setViewCookie(formname + "_" + thiselement.id + "_col_" + i + "_v2", width);
                            }
                        }
                    }
                }
        ...
        if (!this.resizeset) {
            ...
            for (let i = 0; i < this.headerths.length; i++) {
                ...
                let formname = document.body.hasAttribute('entitytypename') ? document.body.getAttribute('entitytypename') : document.body.getAttribute('formtypename');
                let savedlength = getViewCookie(formname + "_" + this.id + "_col_" + i + "_v2");
                if (savedlength) {
                    //debugger;
                    //setViewCookie(formTypeName + "_" + originalFormCaption + "_" + this.id + "_col_" + i, savedlength);//to prolong the expiry date
                    __j(thdiv).outerWidth(savedlength);
                }
    ...
    createControls() {
        ...
        gridcontrolsdiv.setAttribute("class", 'grid-controls-controls');
        ...
        this.selectalldiv = document.createElement('button');
        this.selectalldiv.setAttribute("id", this.id + "_selectalldiv");
        this.selectalldiv.setAttribute("class", "grid-selectalldiv");
        this.selectalldiv.innerText = "Select All";
        this.selectalldiv.addEventListener("click", function (event) {
            //thiselement.multiSelectMode(!thiselement.multiselectmode);
            //thiselement.dispatchMultiSelectEvent(!thiselement.multiselectmode);
            thiselement.dispatchSelectAllEvent();
        });

        this.deselectalldiv = document.createElement('button');
        this.deselectalldiv.setAttribute("id", this.id + "_deselectalldiv");
        this.deselectalldiv.setAttribute("class", "grid-deselectalldiv");
        this.deselectalldiv.innerText = "De-Select All";
        this.deselectalldiv.addEventListener("click", function (event) {
            //thiselement.multiSelectMode(!thiselement.multiselectmode);
            //thiselement.dispatchMultiSelectEvent(!thiselement.multiselectmode);
            thiselement.dispatchDeSelectAllEvent();
        });
        ...
        datainfodiv.setAttribute('class', 'app-grid-data-info-div');
        datainfodiv.setAttribute('id', this.id + "_data_info_div");
        ...
        gridcontrolsdiv.appendChild(this.selectalldiv);
        gridcontrolsdiv.appendChild(this.deselectalldiv);
        ...
        controlsdiv.appendChild(datainfodiv);

        this.headcontainer.appendChild(controlsdiv);
    }

    moveNext() {
        let pageno = this.selectedpageno;
        let rowno = this.selectedrowno;
        if (rowno >= this.alldata[pageno].length - 1) {
            pageno++;
            rowno = 0;
        }
        else {
        ...
        if (pageno > 0) {
            if (!this.multiselectmode) {
                this.dispatchSelectionChangedEvent(pageno, rowno);
            }
            this.scrollToRowIfNeeded(pageno, rowno);
        }
    }
    ...
    movePrevious() {
        ...
        if (rowno == 0) {
            pageno--;
            rowno = this.alldata[pageno].length - 1; // this.pagelength - 1;
        }
        else {
            rowno--;
        }
...
class AppStringEdit extends AppValueElement {
    constructor() {
        ...
        this.lookupbuttonelement.addEventListener('click', function (event) {
            ...
            if (thiselement.lookupentity) {
                thiselement.lookupbuttonelement.setAttribute("lookupentity", thiselement.lookupentity);
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("lookupentity");
            }
            if (thiselement.lookupfield) {
                thiselement.lookupbuttonelement.setAttribute("lookupfield", thiselement.lookupfield);
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("lookupfield");
            }
            if (thiselement.hasAttribute("entityindex")) {
                thiselement.lookupbuttonelement.setAttribute("entityindex", thiselement.getAttribute("entityindex"));
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("entityindex");
            }
            if (thiselement.hasAttribute("fieldname")) {
                thiselement.lookupbuttonelement.setAttribute("fieldname", thiselement.getAttribute("fieldname"));
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("fieldname");
            }
            thiselement.lookupbuttonelement.setAttribute("app-string-edit-id", thiselement.id);
            if (thiselement.hasAttribute("lookupcriteria")) {
                thiselement.lookupbuttonelement.setAttribute("lookupcriteria", thiselement.getAttribute("lookupcriteria"));
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("lookupcriteria");
            }
            if (thiselement.hasAttribute('edt')) {
                thiselement.lookupbuttonelement.setAttribute("edt", thiselement.getAttribute('edt'));
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("edt");
            }
            if (thiselement.hasAttribute("loadableelement")) {
                thiselement.lookupbuttonelement.setAttribute("loadableelement", thiselement.getAttribute("loadableelement"));
            }
            else {
                thiselement.lookupbuttonelement.removeAttribute("loadableelement");
            }
...
class AppStringDisplay extends AppValueElement {
    constructor() {
        ...
        this.__lookupcontextmenu = function (event) {
            ...
            let contextmenuwidth = __j(contextmenu).outerWidth();
            let contextmenuheight = __j(contextmenu).outerHeight();

            let left = x;
            let top = y;
            let right = x + contextmenuwidth;
            let bottom = y + contextmenuheight;

            //debugger;

            if (right > __j(self.frameElement).innerWidth()) {
                left -= right - __j(self.frameElement).innerWidth();
            }
            if (bottom > __j(self.frameElement).innerHeight()) {
                top -= bottom - __j(self.frameElement).innerHeight();
            }
    ...
    setValue(value) {
        this.spanelement.setAttribute('title', value);
...
class AppNumDisplay extends AppStringDisplay {
    ...
    setValue(v) {
        ...
        let textvalue = Number(floatvalue).toLocaleString(undefined, { maximumFractionDigits: this.decimals <= 20 ? this.decimals : 20 });
...
class AppTextDisplay extends AppTextEdit {
    ...
    set value(v) {
        super.value = v;
        this.inputelement.setAttribute('title', v);
    }
}
...
class AppDateDisplay extends AppDateEdit {
    ...
    set value(v) {
        ...
        this.inputelement.setAttribute('title', this.datevalue.toDateString());
    }
}
...
class AppDateTimeDisplay extends AppDateTimeEdit {
    ...
    set value(v) {
        ...
        this.inputelement.setAttribute('title', this.datevalue.toDateString() + " " + this.datevalue.toLocaleTimeString());
    }
}
...
class AppFileEdit extends AppValueElement {
    constructor() {
        ...
        this.deletelink.innerText = 'Delete';
        this.deletelink.setAttribute('href', 'javascript:void(0)');
        let deletespan = document.createElement('span');
        deletespan.setAttribute('class', 'file_empty')
        deletespan.appendChild(this.deletelink);
        //this.attachShadow({ mode: 'open' });
        this.shadowRoot.appendChild(this.fileelement);
        this.shadowRoot.appendChild(deletespan);
        let thiselement = this;
        this.fileelement.addEventListener("change", function (event) {
            ...
            if (this.files && this.files.length > 0) {
                ...
                if (this.files[0].size <= 200 * 1024 * 1024)//200MB
                {
                    ...
                    reader.onload = function () {
                        ...
                        newevent.initCustomEvent("change", false, false, null);
                        thiselement.dispatchEvent(newevent);
                        thiselement.removeAttribute("fileuploaded");
                        //alert('file loaded');
                    };
                    ...
                    thiselement.value = "";
                    //thiselement.mimetype = "";
                    var formData = new FormData();
                    let uploadtimestamp = new Date().getTime();
                    // add assoc key values, this will be posts values
                    formData.append("file", this.files[0], this.files[0].name);
                    formData.append("controlid", thiselement.id);
                    ...
                    __j.ajax({
                        ...
                        url: "/Home/FileUpload",
                        xhr: function () {
                            var myXhr = __j.ajaxSettings.xhr();
                            if (myXhr.upload) {
                        ...
                        success: function (data) {
                            // your callback here
                            unpauseWindow();
                            thiselement.setAttribute("fileuploaded", "");
                            thiselement.setAttribute("timestamp", uploadtimestamp);
                            thiselement.setAttribute("filename", thiselement.fileelement.files[0].name);
                        },
        ...
        this.deletelink.addEventListener('click', function (event) {
            thiselement.value = '';
            thiselement.fileelement.value = '';
            thiselement.setAttribute("mimetype", '');
            thiselement.setAttribute("filename", '');
            thiselement.setAttribute("fileext", '');
        });
    ...
    attributeChangedCallback(attrName, oldVal, newVal) {
        switch (attrName) {
            case "mimetype":
                //break;
                if (newVal) {
                    this.fileelement.setAttribute("accept", newVal);
                }
                else {
