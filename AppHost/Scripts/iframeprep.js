...
for (var i = 0; i < grids.length; i++) {
    ...
    grid.registerHook("createRow", function (tr) {
        ...
        //console.log(tr);
        //debugger;
        //let tempgrid = tr.parentNode.parentNode;
        if (!this.noedit) {
            var button = tr.querySelector('.grid_edit_save_deleted_div > .grid_edit_button');
            button.setAttribute('title', button.innerText);
            button.innerText = '';
            $(button).addClass('fa');
            $(button).addClass('fa-edit');

            button = tr.querySelector('.grid_edit_save_deleted_div > .grid_save_button');
            button.setAttribute('title', button.innerText);
            button.innerText = '';
            $(button).addClass('fa');
            $(button).addClass('fa-save');

            button = tr.querySelector('.grid_edit_save_deleted_div > .grid_details_button');
            button.setAttribute('title', button.innerText);
            button.innerText = '';
            $(button).addClass('fa');
            $(button).addClass('fa-file');

            //button = tr.querySelector('.grid_edit_save_deleted_div > .grid_delete_buton');
            //button.setAttribute('title', button.innerText);
            //button.innerText = '';
            //$(button).addClass('fa');
            //$(button).addClass('fa-times');
            //for (var j = 0; j < buttons.length; j++) {
            //    buttons[j].setAttribute('title', buttons[j].innerText);
            //    $(buttons[j]).addClass('fa');
            //    $(buttons[j]).addClass('fa-delete');
            //}
            //console.log(buttons);
        }
    });
...
registerCreatedHook("app-operations-control", function (appoperationscontrol) {
    appoperationscontrol.registerHook("createOperations", function (ul) {
        //debugger;
        let grids = document.getElementsByTagName('app-grid');
        if (grids.length > 2) {
            let lis = ul.querySelectorAll('li[level="1"]');
            for (let i = 0; i < lis.length; i++) {
                let li = lis[i];
                let span = li.querySelector('span:first-of-type');

                if (span == null) {
                    //debugger;
                }
                else {
                    let title = span.innerText.trim();

                    span.innerText = title;
                }
            }

            lis = ul.querySelectorAll('li[level="1"]');
            for (let i = 0; i < lis.length; i++) {
                let li = lis[i];
                ...
                let span = li.querySelector('span:first-of-type');
                let innerul = li.querySelector('ul:first-of-type');

                if (span != null) {
                    let title = span.innerText;

                    let otherlis = ul.querySelectorAll('li[level="1"]:not([operationid="' + li.getAttribute('operationid') + '"])');

                    for (let j = 0; j < otherlis.length; j++) {
                        let otherli = otherlis[j];
                        let otherspan = otherli.querySelector('span:first-of-type');
                        if (otherspan != null && span.innerText == otherspan.innerText) {
                            let innerlis = otherli.querySelectorAll('ul > li');

                            for (let k = 0; k < innerlis.length; k++) {
                                let innerli = innerlis[k];
            ...
            for (let i = 0; i < lis.length; i++) {
                ...
                //if (span.innerText == 'Grid Controls' || span.innerText == '') {
                //    span.innerText = '';
                //}
                //else 
                if (span != null && !span.querySelector('.caret')) {//to prevent this being done twice
                    //span.innerHTML = '<a href="javascript:void(0)">' + span.innerHTML + '<span class="caret"></span></a>';
                    ...
                    a.appendChild(caretspan);
                    span.appendChild(a);

                    let innerul = li.querySelector('ul:first-of-type');
                    if (innerul) {
                        $(innerul).css('display', 'none');
                        $(innerul).css('position', 'absolute');
                        $(innerul).css('z-index', '100000');
                        $(innerul).css('background-color', 'var(--primary-background-color)');

                        //let setinnerulwidthtimeout;

                        //let setinnerulwidthfunction = function () {
                        //    //this doesn't always get the correct width initially...so, we will now run it in a timeout
                        //    if (!self.frameElement.opened) {
                        //        clearInterval(setinnerulwidthtimeout);
                        //    }
                        //    else {
                        //        let minimuminnerulwidth = $(li).outerWidth() + 9;
                        //        //debugger;
                        //        if ($(innerul).width() < minimuminnerulwidth) {
                        //            $(innerul).width(minimuminnerulwidth);
                        //        }
                        //    }
                        //};

                        //setinnerulwidthtimeout = setInterval(setinnerulwidthfunction, 1000);
                        //setinnerulwidthfunction();

                        $(li).addClass('collapsed_operation_li');
                        $(innerul).addClass('collapsed_operation_ul');
                        //$(innerul).css('-webkit-padding-start', '0px');
                        //$(innerul).css('list-style', 'none');
                    }

                    let hideinneruifunction = function (event) {
                        //debugger;
                        let path = event.path;
                        if (!path.includes(a) && !path.includes(innerul)) {
                            for (let i = 0; i < path.length; i++) {
                                if (path[i] && path[i].tagName && path[i].tagName.toLowerCase() == 'button') {
                                    return;
                                }
                            }
                            $(innerul).css('display', 'none');
                            document.removeEventListener('mousedown', hideinneruifunction);
                        }
                    };
                    ...
                    a.onclick = function (event) {
                        if ($(innerul).css('display') == 'none') {
                            ...
                            document.addEventListener('mousedown', hideinneruifunction);
                        }
                        else {
                            $(innerul).css('display', 'none');
                        }
                    ...
                    let buttons = innerul.querySelectorAll('button');
                    //debugger;
        ...
        //margin: 0px;
        if (permissionsli) {
            //$(permissionsli).css('position', 'absolute');
            //$(permissionsli).css('right', '0px');
            //$(permissionsli).css('top', '5px');
            ...
            //$(permissionsbutton).css('padding', '0px', 'important');
